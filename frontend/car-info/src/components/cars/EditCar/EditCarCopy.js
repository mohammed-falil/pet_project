...
let EditCar = () => {
  ...
  let handleUpdate=(event)=>{
    async function fetchData(){
      ...
      try{
        let updateResponse = await CarService.updateCar(state.car,carId);
        if(updateResponse){
          navigate("http://localhost:9090/admin/car/information",{replace:true});
        }
      }
      catch(error){
        setState({...state,errorMessage:error.message});
        navigate(`http://localhost:9090/admin/car/information/{carId}`,{replace:false})
      }
    }
    fetchData();
  };
